var play = {
	...
	//	this.explosions.createMultiple(10, 'kaboom');
		this.explosions.createMultiple(10, 'blast');
	...
	//	game.physics.arcade.overlap(game.ufo.ship, this.obstacles, this.killPlayer, null, this)
		...
		game.physics.arcade.overlap(game.ufo.laser, this.obstacles, this.destroyItem, null, this);

		
		spawn_rate = 50-game.global.score;				// how fast to add new obstacles to screen (smaller value = more obstacles)
		 // how fast should each obstacle move
		 obstacle_speed = game.global.score * 1.5 + 200;
		/* if(limit-50 <= 0){
			obstacle_speed = obstacle_speed + 75;
			limit=0;
		}
 ...
 */
	...
	spawnObstacle: function (x, y, speed) {
		...
		var obstacle = this.obstacles.create(x, y, 'obstacle'+a)
...
console.log(speed);
	...
	this.sound.kill.play('', 0, 0.5, false)
		...
		//Score increments based on the difficulty
		game.global.score += 5;
		limit+=5;
